...
fn priority(s: &String) -> i32 {
    ...
    let indices = s.match_indices("1").map(|(i, _)| i).collect::<Vec<usize>>();
    for index in indices.iter() {
        checked[*index] = true;
    }

    let wildcards = indices.len();
    ...
    group_sizes.sort();
    let last = group_sizes.pop().unwrap_or(0);
    ans += (last + wildcards) * (last + wildcards);

    for i in group_sizes.iter() {
        ans += i * i;
    }
